...
namespace ouroboros
{
	namespace detail
	{
		...
		group<var_field> build_tree()
		{
			...
			result.add(new base_string("main", "This is the main text.", "", "", 10, std::pair<std::size_t, std::size_t>(0, 10)));
			result.add(g1);
			g1->add(g2);
			g2->add(new base_string("i1", "!!!", "", "", 10, std::pair<std::size_t, std::size_t>(0, 10)));
